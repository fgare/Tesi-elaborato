\chapter{Stato dell'arte}
	...
	In ambito di sviluppo e deployment di applicazioni distribuite si distinguono diversi metodi, ognuno con i propri peculiari vantaggi e svantaggi; descriveremo nel corso di questo capitolo pro e contro di ogni approccio.
...
\section{Architettura monolitica}
	...
	Un simile approccio è ideale per applicazioni di dimensioni ridotte e che non devono soddisfare un elevato numero di richieste, presentano infatti una minore complessità di sviluppo ed offrono performance migliori\cite{site:archComparison}.
	...
	\begin{itemize}
		...
		\item \emph{Poca flessibilità:}
			...
			Alcuni vincoli potrebbero essere la scelta di un linguaggio di programmazione unico, di un framework specifico, di un unico sistema per la gestione del codice sorgente, di un'unica piattaforma di hosting.
		...
		\item \emph{Mantenimento:}
			Anche una singola modifica all'interno di un modulo costringe ad eseguire nuovamente il deploy dell'intera applicazione.
	\end{itemize}
	...
	\begin{figure}[tbp]
		...
		\end{subfigure}
		\caption{Confronto di una struttura monolitica e una a microservizi\cite{site:archComparison} }
	\end{figure}
...
\section{Deployment tradizionale}
	...
	Sebbene semplice ed immediato questo modo di procedere non è ottimale: non permettendo di scalare l'applicazione con dei limiti sull'utilizzo di risorse (CPU, RAM, disco,\dots) si corre il rischio che una seconda applicazione non riesca a garantire le prestazioni desiderate.
...
\section{Deployment su macchine virtuali}
	...
	Le macchine virtuali sono pacchetti software piuttosto pesanti in grado di eseguire l'emulazione dei dispositivi hardware di basso livello quali CPU, RAM, dischi, schede di rete.
	...
	Un simile approccio è in grado di far fronte ad alcune delle problematiche del deployment tradizionale quali:
	
	\begin{itemize}
		\item \emph{Limitazione delle risorse:}
			In funzione di quanto detto sopra è possibile limitare le risorse disponibili per ad una macchina virtuale in termini di  tempo di CPU, quantità di RAM, etc.
		\item \emph{Isolamento:}
			Le macchine virtuali eseguono in un ambiente isolato e completamente autonomo che le rende immuni a tentativi di interferenza da parte di altre macchine virtuali in esecuzione sullo stesso host.
	\end{itemize}
	
	Dall'altro canto i principali svantaggi delle macchine virtuali riguardano la ridotta velocità di generazione e distribuzione delle stesse e il maggiore spazio di archiviazione richiesto:
...
\section{Deployment su container}
	...
	\begin{figure}[tbp]
		...
		\caption{Struttura di un sistema basato su macchine virtuali e container \cite{site:container_vs_VM}}
