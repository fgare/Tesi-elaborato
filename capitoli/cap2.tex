...
\section{Architettura monolitica}
	...
	\begin{figure}[tbp]
			\centering
			\includegraphics[width=0.5\textwidth]{img/cap2/Monolithic_architecture.png}
			\caption{Schema di funzionamento di un'applicazione monolitica\cite{site:archComparison}}
			\label{fig:schemaMonol}
	\end{figure}
	
	\begin{figure}
			\centering
			\includegraphics[width=0.5\textwidth]{img/cap2/Microservice_architecture.png}
		\caption{Schema di funzionamento di un'applicazione a microservizi\cite{site:archComparison} }
		\label{fig:schemaMicros}
	\end{figure}
...
\section{Deployment su macchine virtuali}
	...
	Il deployment su macchine virtuali (Figura \ref{fig:VM_vs_container}), anche chiamato deployment virtualizzato, prevede l'esecuzione di più macchine virtuali sul server fisico, ciascuna delle quali esegue un sistema operativo che a sua volta esegue una o più applicazioni.
	...
	Esistono un gran numero di piattaforme online che permettono l'acquisto di macchine virtuali in cloud, permettendone di dimensionare ciascuna macchina sulla base delle esigenze del cliente.
	I servizi più famosi sono senz'altro Amazon Web Services (AWS), Microsoft Azure e Google Cloud Platform (GCP).
	
	Negli ultimi anni sono anche nati diversi sistemi che permettono di automatizzare la generazione, la configurazione e la gestione di macchine virtuali;
	essi permettono di dichiarare lo stato di esecuzione che si desidera avere e si occupano di mantenere questo stato nel tempo.
	Rientrano in questa categoria strumenti quali Ansible, Vagrant e Terraform.
...
\section{Deployment su container}
	...
	Al pari delle macchine virtuali i container rappresentano un ambiente isolato, con risorse limitate ed un proprio file system per l'esecuzione del software ma non virtualizzano un intero sistema operativo (Figura \ref{fig:VM_vs_container}).
	...
	Per finire molti sistemi runtime per container mettono a disposizione degli sviluppatori repository pubbliche con immagini preconfezionate di software popolari, il tutto al fine di avere uno sviluppo ancora più rapido (Figura \ref{fig:schemaMicros})
	\cite{doc:kube, site:container_vs_VM}.
	
	Docker è sicuramente il più popolare tra i sistemi sviluppati per offrire la virtualizzazione su container, ed è anche merito di questo progetto \emph{open-source} se lo sviluppo su container si è diffuso così tanto negli ultimi anni.
	Spesso utilizzato come alternativa a Docker è Podman (Pod Manager), il quale pone maggior accento sull'aspetto della sicurezza\cite{site:podman}.
