...
\subsection{PostgreSQL}
	...
	Si tratta di una valida alternativa ad altri progetti sia liberi che con licenza ed è divenuto celebre nel tempo per alcuni suoi punti di forza quali la programmabilità, permettendo un incremento nel livello di astrazione, affidabilità e prestazioni raggiunte\cite{doc:postgresql, wiki:postgresql}.
...
\subsection{Kubernetes}
	...
	La necessità di orchestratori di container nasce in contesti di produzione, laddove c'è la necessità di gestire un gran numero di container ed assicurarsi che i servizi offerti siano sempre attivi\cite{doc:docker-overview}.
	...
	\begin{itemize}
		...
		\item \emph{Configurazione dichiarativa}:
			...
			La fase di transizione dallo stato attuale del cluster allo stato desiderato è del tutto trasparente all'utente e non è in alcun modo modificabile, sarà Kubernetes a prendere tutte le decisioni in merito ed evolvere il sistema.
	\end{itemize}
...
\paragraph{Service}
	Un ogetto \emph{Service} rappresenta un'astrazione al fine di esporre un gruppo di pod sulla rete.
	...
	da ciò consegue che non è possibile conoscere quali unità sono attive e funzionanti in un dato momento.
	La configurazione di un service contiene il nome del service, un selettore per identificare i pod raggiungibili e i numeri di porta accessibili dall'esterno, generando di fatto un'astrazione dagli indirizzi IP interni di un insieme di pod\cite{doc:kube-service}.
...
\subsection{API REST}
	...
	\begin{enumerate}
		\item \emph{Interfaccia uniforme}:
			Ogni risorsa trasferita dal server deve rispettare delle condizioni di generalità, come: identificazione univoca della risorsa;
		...
		\item \emph{Sistema a più livelli}:
			\`E possibile definire una struttura gerarchica tra livelli, così che all'arrivo di una richiesta il server principale può inoltrare quest'ultima ad altri server di livello inferiore.
...
\subsection{Token JWT}
	...
	JWT (\emph{JSON Web Token}) è uno standard aperto che definisce una modalità compatta ed autocontenuta per trasmettere informazioni tra le parti coinvolte in una comunicazione come oggetti JSON.
	I dati inviati sono firmati digitalmente grazie ad algoritmi a chiave segreta (HMAC) o a chiave pubblica-privata (RSA) e se ne può quindi verificare l'autenticità.
